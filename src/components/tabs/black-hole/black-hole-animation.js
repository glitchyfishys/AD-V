export const BlackHoleAnimation = (function() {
  ...
  return class Animation {
    constructor(context) {
      ...
      const y = (1 - Math.pow(ACTIVE_THRESHOLD, -2)) / (1 - Decimal.pow(BlackHole(1).power, -2).min(1e100).toNumber());
      ...
      holeSize = SEMIMAJOR_AXIS * (1 - eccentricity) * (1 - Decimal.pow(BlackHole(1).power, -2).min(1e100).toNumber());
