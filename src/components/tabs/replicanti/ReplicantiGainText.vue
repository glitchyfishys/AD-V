...
export default {
  ...
  methods: {
    update() {
      ...
      const logGainFactorPerTick = Decimal.divide(getGameSpeedupForDisplay().mul(updateRateMs * (Math.log(player.replicanti.chance + 1))), getReplicantiInterval());
      ...
      const log10GainFactorPerTickUncapped = Decimal.divide(getGameSpeedupForDisplay().mul( updateRateMs *
