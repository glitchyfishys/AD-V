...
export const Laitela = {
  ...
  get maxAllowedDimension() {
    return Glitch.isRunning ? Glitch.laitelamaxdim : 8 - this.difficultyTier;
  },
  ...
  get realityReward() {
    return Math.clampMin(Math.pow(100, this.difficultyTier) *
      Math.pow(360 / player.celestials.laitela.fastestCompletion.toNumber(), 2), 1);
  },
  // Note that entropy goes from 0 to 1, with 1 being completion
  get entropyGainPerSecond() {
    if(Glitch.isRunning) return 0;
  ...
  get darkMatterMult() {
    return new Decimal(this.celestial.darkMatterMult).add(1);
  },
  ...
  annihilate(force) {
    ...
    this.celestial.darkMatterMult = Decimal.add(this.celestial.darkMatterMult,this.darkMatterMultGain);
