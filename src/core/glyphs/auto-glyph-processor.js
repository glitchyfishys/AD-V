export const AutoGlyphProcessor = {
  ...
  filterValue(glyph) {
    ...
    switch (this.scoreMode) {
      ...
      case AUTO_GLYPH_SCORE.EFFECT_SCORE: {
        const effectList = getGlyphEffectsFromBitmask(glyph.effects, glyph.type, 0)
  ...
  pick(glyphs) {
    ...
    return glyphs
      ...
      .reduce((x, y) => ( Decimal.gt(x.score, y.score) ? x : y))
      .glyph;
  },
  getRidOfGlyph(glyph) {
...
export function getGlyphLevelInputs() {
  ...
  scaledLevel = instabilitySoftcap(scaledLevel, 6e7, 50);
  scaledLevel = instabilitySoftcap(scaledLevel, 7e7, 50);
  scaledLevel = instabilitySoftcap(scaledLevel, 8e7, 50);
  scaledLevel = instabilitySoftcap(scaledLevel, 9e7, 50);
  scaledLevel = instabilitySoftcap(scaledLevel, 1e8, 5);
  ...
  return {
    ...
    singularityEffect,
    rawLevel: baseLevel,
    actualLevel: Math.clamp(scaledLevel, 1, Glyphs.levelCap),
  };
}
