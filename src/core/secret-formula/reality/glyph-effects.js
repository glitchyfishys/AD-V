...
export const glyphEffects = {
  ...
  dilationDT: {
    ...
    softcap: value => ((value.gt("1e5000")) ? value.div((value.div("1e5000")).pow(0.97)) : value),
  ...
  dilationgalaxyThreshold: {
    ...
    effect: (level, strength) => 1 / (Math.pow(level, 0.17) * Math.pow(strength, 0.35) * (100 +
      GlyphAlteration.sacrificeBoost("dilation") / 2) / 100),
    formatEffect: x => format(x, 3, 3),
  ...
  infinitypow: {
    ...
    softcap: value => ((value > 6.5) ? Math.min( value / ((value / 6.5) ** 0.95),8) : value),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
  ...
  powerpow: {
    ...
    softcap: value => ((value > 1.8) ? Math.min( value / ((value / 1.8) ** 0.95),1.9) : value),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
    conversion: x => 2 / (x + 1),
  ...
  effarigachievement: {
    ...
    softcap: value => ((value > 15) ? value / ((value / 15) ** 0.95) : value),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
