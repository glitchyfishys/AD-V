...
export const glyphEffects = {
  ...
  timeetermult: {
    ...
    effect: (level, strength) => Decimal.pow(Decimal.add(strength, 3).times(level), 0.9)
  ...
  timeEP: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("time")
      ? "EP ×{value} and ^{value2}"
      : "EP ×{value}"),
    effect: (level, strength) => Decimal.pow(new Decimal(level).times(strength), 3).times(100),
  ...
  dilationDT: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("dilation")
      ...
      : Decimal.pow(new Decimal(level).times(strength), 1.5).times(2)),
    formatEffect: x => format(x, 2, 1),
    combine: effects => {
      const eff = effects.reduce(Decimal.prodReducer, DC.D1);
      return eff.gt("1e5000") ? eff.div(eff.div("1e5000").pow(0.97)) : eff;
    },
  ...
  dilationgalaxyThreshold: {
    ...
    effect: (level, strength) => Decimal.pow(level, 0.17).times(Decimal.pow(strength, 0.35))
      .mul(GlyphAlteration.sacrificeBoost("dilation").div(50).max(1)).add(1).recip(),
    formatEffect: x => format(x, 3, 3),
    ...
    combine: effects => {
      ...
      return prod.lt(0.4)
        ? { value: Decimal.pow(prod, 0.45), capped: true }
  ...
  dilationTTgen: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("dilation")
      ...
      : "{value} TT/hr"),
    effect: (level, strength) => Decimal.pow(new Decimal(level).times(strength), 0.5).div(10000),
  ...
  replicationspeed: {
    ...
    combine: effects => {
      const eff = effects.reduce(Decimal.prodReducer, DC.D1);
      return eff.gt("1e15000") ? eff.div(eff.div("1e15000").pow(0.97)) : eff;
    },
  ...
  infinityIP: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("infinity")
      ? "IP ×{value} and ^{value2}"
      : "IP ×{value}"),
    effect: (level, strength) => Decimal.pow(new Decimal(level).times(Decimal.add(strength, 1)), 6).times(1e4),
    formatEffect: x => format(x, 2, 3),
    combine: GlyphCombiner.multiply,
  ...
  infinityinfmult: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("infinity")
      ? DC.D1_02.pow(level)
      : Decimal.times(level, strength).pow(1.5).times(2)),
    formatEffect: x => format(x, 2, 1),
  ...
  powerdimboost: {
    ...
    effect: (level, strength) => Decimal.pow(Decimal.times(level, strength), 0.5)
  ...
  powerbuy10: {
    ...
    effect: (level, strength) => Decimal.times(level, strength).div(12).add(1),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.addExponents,
    enabledInDoomed: true,
  },
  ...
  effarigglyph: {
    ...
    effect: (level, strength) => Decimal.floor(Decimal.pow(Decimal.times(level, strength), 0.5).times(10)),
  ...
  effarigantimatter: {
    ...
    combine: effects => {
      const eff = effects.reduce(Decimal.prodReducer, DC.D1);
      return eff.gt(1.015) ? eff.div(eff.div(1.015).pow(0.8)).min(1.2) : eff;
    },
  },
  timeshardpow: {
    id: "timeshardpow",
    ...
    effect: (level, strength) => (Decimal.div(strength, 3.5)).times(Decimal.pow(level, 0.35)).div(400).add(1),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x.sub(1), 3, 3),
    combine: GlyphCombiner.addExponents,
    enabledInDoomed: true,
  ...
  realitygalaxies: {
    ...
    totalDesc: "All Galaxy strength +{value}",
    shortDesc: "Galaxy Strength +{value}",
    effect: level => Decimal.pow(new Decimal.div(level, 100000), 0.5).add(1),
  ...
  realityrow1pow: {
    ...
    effect: level => new Decimal(level).div(1.25e5).add(1),
    formatEffect: x => format(x, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  realityDTglyph: {
    ...
    effect: level => Decimal.sqrt(new Decimal(level).div(10)).div(500),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.add,
  },
