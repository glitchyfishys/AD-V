...
export const breakInfinityUpgrades = {
  totalAMMult: {
    ...
    effect: () => Math.pow(player.records.thisMeta.maxAM.exponent + 1, 0.5),
  ...
  slowestChallengeMult: {
    ...
    effect: () => Decimal.clampMin( Decimal.div(50, Time.worstChallenge.totalMinutes), 1),
    formatEffect: value => formatX(value, 2, 2),
    hasCap: true,
    cap: DC.D3E4
  },
  infinitiedGen: {
    ...
    formatEffect: value => {
      if (value.eq(Number.MAX_VALUE) && !Pelle.isDoomed) return "No Infinity generation";
