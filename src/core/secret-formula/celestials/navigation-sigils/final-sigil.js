...
function sigilProgress() {
  const riftProgress = PelleRifts.all.map(r => Decimal.clamp(r.realPercentage, 0, 1)).min().clampMax(1e100).toNumber();
...
const SigilAttributes = {
  visible: () => PelleRifts.all.map(r => Decimal.clamp(r.realPercentage, 0, 1)).min().gt(0),
