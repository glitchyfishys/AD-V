...
export const v = {
  ...
  runUnlocks: [
    ...
    },
    {
      id: 10,
      name: "Revengeance",
      description: value => `Reach ${format(Decimal.pow10(value), 2)} Antimatter in Glitch's Reality with V's Reality Enabled, you also need  ${STreq[VRunUnlock(10).completions]} ST.
       ${VRunUnlock(10).completions > 0 ? (
  ...
  unlocks: {
    ...
    gamespeedPower: {
      ...
      effect: () => V.spaceTheorems.pow(0.66).div(100).add(1),
      format: x => formatPow(x, 2, 2),
      requirement: () => V.spaceTheorems.gte(125),
    },
    RMcap: {
      id: 8,
      reward: () => `Increase the RM cap based on total STs and Glitch Glyphs can be made`,
      description: () => `Have ${formatInt(140)} V-Achievements`,
      effect: () => Decimal.max(V.spaceTheorems.pow(0.05),1),
      format: x => formatPow(x, 2, 2),
    ...
    TScap: {
      ...
      description: () => `Have ${formatInt(165)} V-Achievements and have ${format("e5e17")} Antimatter`,
      effect: () => Decimal.pow(1.1, V.spaceTheorems.sub(130)).max(1),
      format: x => formatX(x, 2, 2),
      requirement: () => V.spaceTheorems.gte(165) && Currency.antimatter.gte("e5e17"),
    },
    glyphCap: {
      id: 10,
      reward: () => `Increase Glyph Level based on STs and double Glyph Refinement cap`,
      description: () => `Have ${formatInt(175)} V-Achievements`,
      requirement: () => V.spaceTheorems.gte(175),
      effect: () => Decimal.pow(V.spaceTheorems.div(100), 0.25).max(1),
      format: x => formatPow(x, 2, 2),
    },
    newStudies: {
      id: 11,
